import type { Question } from "../../../types";

// General aviation knowledge - Operational Procedures.
// Original questions written from general, widely-published aviation
// operational-procedures knowledge, not copied from any single source
// document. No company, brand, or publisher names appear anywhere below.
export const OPERATIONAL_PROCEDURES_QUESTIONS: Question[] = [
  {
    id: 1,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "The risk of dynamic hydroplaning on a runway depends primarily on which factor?",
    options: [
      "The depth of standing water on the runway",
      "The aircraft's total weight",
      "The strength of the headwind component",
      "The amount of speed remaining after lift-off",
    ],
    answer: 0,
    explain:
      "Dynamic hydroplaning occurs when a wedge of standing water builds up faster than it can escape from under the tire, effectively lifting the tire off the runway surface - so the depth of standing water (relative to tire pressure/speed) is the dominant factor, more than aircraft weight.",
    reference: "Operational Procedures - Runway Contamination & Hydroplaning",
  },
  {
    id: 2,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Under a common operational definition, a runway is considered 'damp' when:",
    options: [
      "Its surface is not dry, but surface moisture does not give it a shiny appearance",
      "It is covered with a film of water more than 3 mm deep",
      "It shows a clearly shiny, wet appearance",
      "Any precipitation has been reported within the last hour, regardless of surface condition",
    ],
    answer: 0,
    explain:
      "A damp runway is one whose surface is not dry but isn't covered by a visible film of water giving it a shiny appearance either - that visibly shiny, standing-water condition is instead what distinguishes a 'wet' runway.",
    reference: "Operational Procedures - Runway Surface Condition Definitions",
  },
  {
    id: 3,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "If no specific wet-runway correction factor is published in the aircraft's performance data, landing distance on a wet runway should generally be increased over the dry landing distance by approximately:",
    options: ["15%", "5%", "50%", "100%"],
    answer: 0,
    explain:
      "A commonly applied default correction, absent specific manufacturer data for the type, is to increase the dry landing distance by about 15% to account for reduced wet-runway braking performance.",
    reference: "Operational Procedures - Wet & Contaminated Runway Performance",
  },
  {
    id: 4,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "In still air, the wingtip vortices produced by an aircraft at take-off tend to:",
    options: [
      "Separate and move outward, one to each side of the flight path",
      "Stay perfectly stationary in place over the runway",
      "Travel only in the direction of the prevailing surface wind",
      "Collapse immediately once the gear is retracted",
    ],
    answer: 0,
    explain:
      "With no wind to push them one way, the pair of wingtip vortices trailing an aircraft tend to drift apart, moving outward to each side of the flight path at a few knots, which is part of why crosswind conditions can push one vortex back over the runway.",
    reference: "Operational Procedures - Wake Turbulence Behaviour",
  },
  {
    id: 5,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "For weather-driven alternate-aerodrome planning on a twin-engine aircraft, a commonly applied rule requires a suitable diversion aerodrome to be reachable within approximately:",
    options: [
      "1 hour of flight at single-engine (one-engine-inoperative) cruise speed",
      "2 hours of flight at all-engines cruise speed",
      "30 minutes of flight at all-engines cruise speed",
      "4 hours of flight at single-engine cruise speed",
    ],
    answer: 0,
    explain:
      "A long-standing planning principle for twin-engine aircraft not operating under extended-range approval is that a usable alternate should be reachable within about an hour, flown at the reduced cruise speed available with one engine inoperative - a conservative planning margin distinct from the enroute alternate rules used for extended-range (ETOPS-style) operations.",
    reference: "Operational Procedures - Alternate Aerodrome Planning",
  },
  {
    id: 6,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the maximum validity period generally applied to a SNOWTAM?",
    options: ["24 hours", "3 hours", "12 hours", "6 hours"],
    answer: 0,
    explain:
      "A SNOWTAM reporting runway surface conditions (snow, ice, slush, standing water) is generally valid for up to 24 hours, or until superseded by a fresh report - conditions are expected to be re-assessed and re-published at least that often.",
    reference: "Operational Procedures - Runway Condition Reporting",
  },
  {
    id: 7,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "When an aircraft flies into windshear, which parameter is typically the first to visibly change?",
    options: ["Indicated airspeed", "Groundspeed", "Pitch angle", "Vertical speed"],
    answer: 0,
    explain:
      "A sudden shift in the surrounding air mass immediately changes the airflow over the aircraft, so indicated airspeed reacts first - pitch angle, vertical speed, and groundspeed follow only as the aircraft's flight path actually responds to that airspeed change.",
    reference: "Operational Procedures - Windshear Recognition",
  },
  {
    id: 8,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Flight crew members occupying the flight deck are generally required to keep their safety belt fastened:",
    options: [
      "Whenever they are at their station, not only during take-off and landing",
      "Only during take-off and landing",
      "Only when explicitly instructed by the commander",
      "Only above a specified altitude such as FL200",
    ],
    answer: 0,
    explain:
      "The general rule is that a crew member occupying a flight deck seat keeps the safety belt (and, where fitted, harness) fastened for the whole time they are at that station, not just during the take-off and landing phases.",
    reference: "Operational Procedures - Flight Deck Safety Requirements",
  },
  {
    id: 9,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "A runway braking action report of 0.25 or below is generally classified as:",
    options: ["Poor", "Medium", "Good", "Unreliable, and therefore ignored"],
    answer: 0,
    explain:
      "On the commonly used braking-action reporting scale, a friction coefficient of about 0.25 or lower corresponds to a 'poor' classification, signalling a significantly reduced ability to stop or maintain directional control.",
    reference: "Operational Procedures - Runway Braking Action Reports",
  },
  {
    id: 10,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "For wake turbulence radar separation, when a heavy aircraft is following directly behind another heavy aircraft on approach to the same runway, the applicable minimum is generally about:",
    options: ["7.4 km (4 nm)", "3.7 km (2 nm)", "9.3 km (5 nm)", "11.1 km (6 nm)"],
    answer: 0,
    explain:
      "Heavy-behind-heavy is the one wake-turbulence radar-separation case that uses a reduced 4 nm minimum, reflecting that both aircraft can tolerate somewhat more wake exposure than lighter aircraft following a heavy one, which instead need larger separations.",
    reference: "Operational Procedures - Wake Turbulence Separation Minima",
  },
  {
    id: 11,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "For non-radar wake-turbulence time separation, when a light aircraft is landing behind a medium aircraft, the applicable minimum is generally:",
    options: ["3 minutes", "1 minute", "2 minutes", "5 minutes"],
    answer: 0,
    explain:
      "A light aircraft landing behind a medium aircraft is one of the cases requiring the longer 3-minute non-radar time separation, reflecting the lighter aircraft's greater vulnerability to the preceding aircraft's wake.",
    reference: "Operational Procedures - Wake Turbulence Separation Minima",
  },
  {
    id: 12,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Wake turbulence intensity behind a given aircraft is generally greatest when that aircraft is:",
    options: [
      "Heavy, slow, and in a clean configuration (flaps and gear retracted)",
      "Light, fast, and in a clean configuration",
      "Heavy and fast, regardless of configuration",
      "Light, slow, with flaps and gear extended",
    ],
    answer: 0,
    explain:
      "Wake vortex strength scales with the lift the wing is generating, which is greatest for a heavy aircraft flying slowly in a clean configuration - flaps and gear extended tend to break up and weaken the vortex somewhat, which is part of why the heavy/slow/clean combination is treated as the worst case.",
    reference: "Operational Procedures - Wake Turbulence Behaviour",
  },
  {
    id: 13,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "For a Category IIIA precision approach, what is the general minimum RVR requirement?",
    options: [
      "Not less than about 200 m, with no decision height or a DH below 100 ft",
      "Not less than about 550 m, with a decision height of 200 ft",
      "Not less than about 800 m, with no decision height at all",
      "Not less than about 50 m, with no decision height",
    ],
    answer: 0,
    explain:
      "Category IIIA approaches are generally flown to an RVR of not less than about 200 m, either with no applicable decision height or with a DH lower than 100 ft. The even lower minima associated with Category IIIB require additional certification and are not simply a smaller version of IIIA.",
    reference: "Operational Procedures - Approach Categories & Minima",
  },
  {
    id: 14,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Roughly how long does it take for the wake turbulence generated by a large, wide-body aircraft at take-off to dissipate?",
    options: [
      "On the order of a few minutes",
      "Around 30 seconds",
      "Around 10 hours",
      "It never fully dissipates and must be avoided indefinitely",
    ],
    answer: 0,
    explain:
      "A large aircraft's wake vortices typically break down and dissipate within a few minutes under normal conditions, which is part of the reasoning behind the standard 2-3 minute non-radar time-based wake turbulence separations.",
    reference: "Operational Procedures - Wake Turbulence Behaviour",
  },
  {
    id: 15,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "In the event of severe smoke in the cabin, is it useful and recommended for passengers to use the supplied oxygen masks?",
    options: [
      "Yes - it is possible and recommended, since the supplied oxygen itself is not made unsafe by surrounding smoke",
      "No - using oxygen in the presence of smoke is always explosive and must be avoided",
      "No - the oxygen units automatically stop supplying oxygen whenever smoke is detected",
      "It has no effect either way and is neither helpful nor harmful",
    ],
    answer: 0,
    explain:
      "Using passenger oxygen during a smoke event is both possible and recommended - the toxic smoke in the cabin doesn't get mixed into the oxygen supply itself, so the mask still delivers breathable oxygen even while ambient cabin air is contaminated.",
    reference: "Operational Procedures - Cabin Smoke & Emergency Oxygen",
  },
  {
    id: 16,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "An aircraft suffers an explosive decompression while cruising at a high flight level. What is generally the crew's first priority action?",
    options: [
      "Don oxygen masks",
      "Transmit a distress (MAYDAY) call",
      "Disconnect the autopilot",
      "Reassure the passengers over the PA system",
    ],
    answer: 0,
    explain:
      "Immediate donning of oxygen masks takes priority, since useful time of consciousness at high altitude after a decompression can be very short - other actions such as beginning a descent, notifying ATC, and managing the cabin follow once the crew themselves are safely on oxygen.",
    reference: "Operational Procedures - Decompression Emergency Procedures",
  },
  {
    id: 17,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Once an aircraft has accelerated beyond V1 during the take-off roll, what is the crew generally expected to do if an engine then fails?",
    options: [
      "Continue the take-off, since stopping safely within the remaining distance is no longer assured",
      "Reject the take-off immediately regardless of speed",
      "Immediately shut down the remaining engine as well",
      "Continue accelerating past VR without rotating",
    ],
    answer: 0,
    explain:
      "V1 is defined so that stopping safely within the accelerate-stop distance is no longer assured once it is exceeded, so the standard procedure after V1 is to continue the take-off and handle the failure airborne, rather than attempt a late rejected take-off.",
    reference: "Operational Procedures - Rejected Take-off Decision",
  },
  {
    id: 18,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is generally meant by a 'stabilized approach'?",
    options: [
      "An approach flown in the landing configuration, on the correct flight path and speed, with a steady rate of descent, by a defined height above the runway",
      "Any approach flown with the autopilot engaged throughout",
      "An approach where the aircraft is exactly on the ILS glideslope only at the point of touchdown",
      "An approach where power is kept at idle from the top of descent onward",
    ],
    answer: 0,
    explain:
      "A stabilized approach requires the aircraft to be in the correct landing configuration, on the intended flight path, at an appropriate speed, with a steady rate of descent and appropriate power setting, established by a defined height (commonly 1,000 ft in instrument conditions or 500 ft in visual conditions) - failing to meet these criteria is a standard trigger for a go-around.",
    reference: "Operational Procedures - Stabilized Approach Criteria",
  },
  {
    id: 19,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Up until what point during a landing can a go-around generally still be initiated?",
    options: [
      "At any time up until touchdown (and in some cases even after touchdown, if certified for it)",
      "Only before passing the outer marker",
      "Only above 1,000 ft above the runway",
      "Only if requested in advance from air traffic control",
    ],
    answer: 0,
    explain:
      "A go-around remains an available option at any point up until touchdown - and some aircraft types are certified for a go-around initiated even after the main gear briefly touches down - reflecting that safety takes priority over completing a landing that isn't developing as expected.",
    reference: "Operational Procedures - Go-Around Decision",
  },
  {
    id: 20,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Under the commonly applied 'sterile flight deck' rule, non-essential conversation and activity are generally restricted during which phases of flight?",
    options: [
      "Below a specified altitude (commonly 10,000 ft) during climb and descent, and during taxi",
      "Only during the cruise phase at high altitude",
      "Only when passengers are being served a meal",
      "At all times throughout the entire flight equally",
    ],
    answer: 0,
    explain:
      "The sterile flight deck concept restricts flight crew conversation and activity to only what's essential for safely operating the aircraft during critical phases - typically taxi, take-off, climb, approach, and landing below a specified altitude such as 10,000 ft - reducing distraction during the periods with the least margin for error.",
    reference: "Operational Procedures - Sterile Flight Deck",
  },
  {
    id: 21,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "In runway contamination reporting, what does 'slush' generally refer to?",
    options: [
      "Partially melted snow or ice with a high water content, of a consistency that splashes when stepped on",
      "Freshly fallen, dry powder snow with no melting",
      "A thin, invisible film of ice on the surface",
      "Compacted snow that has been packed down by aircraft and vehicle traffic",
    ],
    answer: 0,
    explain:
      "Slush is water-saturated snow that, due to its high water content, splashes when stamped on firmly - it is treated differently from dry snow or compacted snow in performance calculations because of its higher drag and hydroplaning-type risk at speed.",
    reference: "Operational Procedures - Runway Contamination Reporting",
  },
  {
    id: 22,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general purpose of a Minimum Equipment List (MEL)?",
    options: [
      "To define the conditions under which an aircraft may legally be dispatched with certain inoperative equipment",
      "To list the equipment required only for maintenance training purposes",
      "To specify the minimum crew qualifications required for a given aircraft type",
      "To set out passenger cabin service standards",
    ],
    answer: 0,
    explain:
      "A Minimum Equipment List sets out which items of equipment may be inoperative for dispatch, and under what conditions or operational limitations, without compromising the required level of safety - it does not mean any listed item may simply be ignored, but defines a controlled, approved way of managing it.",
    reference: "Operational Procedures - Minimum Equipment List",
  },
  {
    id: 23,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Under what general circumstances might an aircraft need to dump (jettison) fuel before landing?",
    options: [
      "When it needs to reduce weight quickly to get below its maximum landing weight for an early, urgent landing",
      "Whenever a flight is diverted for any reason, regardless of weight",
      "As a routine step before every normal landing",
      "Only to correct a lateral fuel imbalance between tanks",
    ],
    answer: 0,
    explain:
      "Fuel jettison (where the aircraft type is equipped for it) is generally used to reduce aircraft weight quickly when an early, urgent landing is required and the aircraft would otherwise be above its structural maximum landing weight - not a routine procedure, and not the standard way to correct a simple tank imbalance.",
    reference: "Operational Procedures - Fuel Jettison",
  },
  {
    id: 24,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "During an emergency descent following a decompression, what is generally the crew's priority order of actions?",
    options: [
      "Don oxygen masks, then initiate the descent, then notify air traffic control",
      "Notify air traffic control, then don oxygen masks, then descend",
      "Initiate the descent first, then don oxygen masks once level",
      "Brief the cabin crew first, before taking any other action",
    ],
    answer: 0,
    explain:
      "Because useful time of consciousness can be very short at high altitude following a decompression, the crew's own oxygen masks go on first, before beginning the emergency descent - notifying air traffic control and further cabin actions follow once the aircraft is safely descending and the crew are protected.",
    reference: "Operational Procedures - Emergency Descent Procedures",
  },
  {
    id: 25,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "When an evacuation is ordered, what is generally the first priority communicated to the cabin?",
    options: [
      "A clear, direct evacuation command instructing passengers to leave immediately, leaving belongings behind",
      "A detailed explanation of what caused the emergency",
      "Individual seat-by-seat boarding-style instructions",
      "A request for passengers to first collect their carry-on baggage",
    ],
    answer: 0,
    explain:
      "An evacuation command is kept short, direct, and repeated (such as instructions to release belts, leave everything behind, and move to the exits) so it is immediately understood and acted on under stress - passengers taking time to gather baggage is a well-documented factor that has slowed real evacuations and increased risk.",
    reference: "Operational Procedures - Emergency Evacuation",
  },
  {
    id: 26,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Which of the following would generally be considered a factor in deciding whether to divert to an alternate aerodrome?",
    options: [
      "Deteriorating weather at the destination, a technical issue, insufficient fuel to hold, or a medical emergency onboard",
      "Only a change in the forecast wind direction at cruise altitude",
      "The time of day alone, regardless of any other factor",
      "Only a request from the cabin crew for a meal-service delay",
    ],
    answer: 0,
    explain:
      "A diversion decision generally weighs several factors together - deteriorating destination weather, a technical problem affecting continued safe flight, fuel state relative to holding or further delay, and onboard medical emergencies - rather than any single, narrow trigger.",
    reference: "Operational Procedures - Diversion Decision Factors",
  },
  {
    id: 27,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "In a crosswind landing, what is the general purpose of aligning the aircraft's longitudinal axis with the runway before or at touchdown (whether by the crab, wing-low, or a combined technique)?",
    options: [
      "To avoid excessive side loads on the landing gear and maintain directional control as the aircraft touches down",
      "To increase the aircraft's groundspeed at touchdown",
      "To reduce the aircraft's stall speed during the flare",
      "To comply with a requirement that applies only above a specific crosswind component",
    ],
    answer: 0,
    explain:
      "Landing with the aircraft's longitudinal axis skewed relative to the runway direction imposes side loads on the landing gear and increases the risk of a directional control upset, so crosswind techniques (whether crabbing until just before touchdown, holding a wing low with opposite rudder, or a blend of both) are aimed at aligning the aircraft with the runway at touchdown.",
    reference: "Operational Procedures - Crosswind Landing Technique",
  },
  {
    id: 28,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "If a TCAS resolution advisory (RA) conflicts with a simultaneous instruction from air traffic control, which is the crew generally expected to follow?",
    options: [
      "The TCAS resolution advisory",
      "The air traffic control instruction",
      "Neither; the crew should ask the other pilot to decide",
      "Whichever instruction was received first, regardless of source",
    ],
    answer: 0,
    explain:
      "A TCAS resolution advisory is generally treated as taking priority over a conflicting ATC instruction, since it reflects an imminent, close-in traffic conflict that the automated system has already resolved using both aircraft's TCAS logic - ATC may not even be aware of the specific conflict driving the RA.",
    reference: "Operational Procedures - TCAS Resolution Advisories",
  },
  {
    id: 29,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is a de-icing/anti-icing fluid's 'holdover time' (HOT)?",
    options: [
      "The estimated period during which the fluid is expected to prevent frost, ice, or snow from accumulating on the treated surfaces",
      "The maximum time the fluid can remain in its storage tank before it must be discarded",
      "The time it takes for the fluid to be fully absorbed into the airframe structure",
      "A fixed period that never varies regardless of the weather",
    ],
    answer: 0,
    explain:
      "Holdover time is an estimated, condition-dependent window (varying with precipitation type, intensity, and temperature) during which applied de-icing/anti-icing fluid is expected to keep protected surfaces free of frozen contamination - once that time is exceeded, the protection can no longer be relied on and re-treatment or a fresh inspection is generally required before takeoff.",
    reference: "Operational Procedures - De-icing & Anti-icing Procedures",
  },
  {
    id: 30,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general purpose of activating Low Visibility Procedures (LVPs) at an aerodrome?",
    options: [
      "To provide extra protection (such as safeguarding sensitive ILS signal areas and increasing separation) needed to support low-visibility approach and landing operations",
      "To close the aerodrome entirely to all traffic until visibility improves",
      "To increase the maximum permitted taxi speed for departing aircraft",
      "To cancel all instrument approach procedures until visibility improves",
    ],
    answer: 0,
    explain:
      "Low Visibility Procedures introduce additional protections - such as safeguarding the ILS critical/sensitive areas from vehicle or aircraft intrusion and increasing spacing between movements - that are needed to support the lower-minima approach and landing operations (Category II/III) that become necessary once visibility or RVR drops below defined thresholds.",
    reference: "Operational Procedures - Low Visibility Procedures",
  },
  {
    id: 31,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "How does an 'explosive' decompression generally differ from a slow (gradual) decompression in terms of how quickly it develops?",
    options: [
      "An explosive decompression occurs almost instantaneously (often under a second), while a slow decompression develops gradually and may go unnoticed without a cabin altitude warning",
      "Both types occur over several minutes, with no meaningful difference in speed",
      "A slow decompression is always more dangerous because it gives no warning at all",
      "An explosive decompression can only happen on the ground, never at altitude",
    ],
    answer: 0,
    explain:
      "An explosive decompression - typically from a large, sudden structural opening - equalises cabin and outside pressure almost instantly, whereas a slow decompression develops gradually enough that, without an automated cabin altitude warning, the crew and passengers might not immediately notice the pressure loss until symptoms of hypoxia begin to appear.",
    reference: "Operational Procedures - Decompression Emergency Procedures",
  },
  {
    id: 32,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general distinction between 'fuel exhaustion' and 'fuel starvation'?",
    options: [
      "Fuel exhaustion means no usable fuel remains onboard; fuel starvation means usable fuel exists onboard but fails to reach the engine(s)",
      "The two terms describe exactly the same situation and are used interchangeably",
      "Fuel exhaustion refers only to a fuel leak, while fuel starvation refers only to a fuel-gauge malfunction",
      "Fuel starvation can only occur on the ground, never in flight",
    ],
    answer: 0,
    explain:
      "Fuel exhaustion means the aircraft has genuinely run out of usable fuel; fuel starvation means fuel is still present onboard but, due to a blocked line, mismanaged tank selection, or a similar issue, is not reaching the engine(s) that need it - an important distinction for both troubleshooting in flight and post-incident investigation.",
    reference: "Operational Procedures - Fuel Management",
  },
  {
    id: 33,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is generally meant by a 'runway incursion'?",
    options: [
      "Any unauthorised presence of an aircraft, vehicle, or person on the protected area of a surface designated for landing and takeoff",
      "Any aircraft landing long on a runway with adequate remaining distance",
      "A normal, authorised runway crossing by a taxiing aircraft with an ATC clearance",
      "A go-around performed for any reason",
    ],
    answer: 0,
    explain:
      "A runway incursion is any incorrect or unauthorised presence of an aircraft, vehicle, or person within the protected runway area - a category of safety event distinct from a normal, properly cleared and authorised runway crossing or occupation.",
    reference: "Operational Procedures - Runway Safety",
  },
  {
    id: 34,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "During the landing roll, at what point is reverse thrust generally most effective at helping decelerate the aircraft?",
    options: [
      "At higher groundspeed, shortly after touchdown, with effectiveness reducing as the aircraft slows",
      "At very low groundspeed, just before the aircraft comes to a stop",
      "Reverse thrust effectiveness is constant regardless of groundspeed",
      "Reverse thrust is only effective once the aircraft has already stopped",
    ],
    answer: 0,
    explain:
      "Reverse thrust contributes the most stopping benefit at higher groundspeeds shortly after touchdown, when it is redirecting a large mass of fast-moving air; its effectiveness diminishes as the aircraft slows, which is why wheel braking (largely unaffected by speed in the same way) becomes progressively more important later in the landing roll.",
    reference: "Operational Procedures - Reverse Thrust Use",
  },
  {
    id: 35,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Following a suspected bird strike, what is generally the recommended course of action?",
    options: [
      "Report the strike and have the aircraft inspected for damage before further flight, particularly if an engine or other critical component may have been affected",
      "No action is needed if the flight controls still respond normally",
      "Simply note it in the logbook at the end of the flying day with no inspection required",
      "Only take action if the strike happened during the landing roll, not during takeoff or climb",
    ],
    answer: 0,
    explain:
      "A suspected bird strike is generally reported and followed by an inspection for damage - especially to engines, which can suffer serious internal damage from ingestion that isn't necessarily obvious from the flight deck - before the aircraft is considered fit for further flight.",
    reference: "Operational Procedures - Bird Strike Procedures",
  },
  {
    id: 36,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "During a go-around with one engine inoperative, what additional demand does asymmetric thrust generally place on the crew compared with an all-engines go-around?",
    options: [
      "Increased directional control demands (rudder input to counter the yawing moment) alongside reduced climb performance",
      "No additional demand at all; performance and handling are unaffected by the inoperative engine",
      "A requirement to reduce flap setting immediately, regardless of speed",
      "A requirement to shut down the remaining engine as well for safety",
    ],
    answer: 0,
    explain:
      "With one engine inoperative, the asymmetric thrust from the remaining engine(s) produces a yawing moment that must be countered with rudder to maintain directional control, on top of which the aircraft has reduced total available thrust and therefore reduced climb performance compared with an all-engines-operating go-around.",
    reference: "Operational Procedures - Asymmetric Go-Around Considerations",
  },
  {
    id: 37,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Below V1, under what general circumstances would a crew normally reject the take-off?",
    options: [
      "Only for genuinely serious conditions, such as an engine failure/fire, a fire warning, or a sudden loss of directional control - not for any and every abnormal indication",
      "For absolutely any abnormal indication, however minor, regardless of speed",
      "Only if instructed to do so by air traffic control",
      "Only if a cabin crew member requests it over the interphone",
    ],
    answer: 0,
    explain:
      "Rejecting a take-off is itself not risk-free, especially at higher speed, so standard guidance reserves a rejected take-off for genuinely serious conditions - an engine failure or fire, a master warning, or a clear loss of directional control - rather than treating every minor abnormal indication as automatic grounds to reject, which is why type-specific training defines what does and doesn't warrant a reject.",
    reference: "Operational Procedures - Rejected Take-off Decision",
  },
  {
    id: 38,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is generally meant by an 'approach ban'?",
    options: [
      "A regulatory rule preventing an aircraft from continuing an approach beyond a defined point when the reported visibility or RVR is below the minimum applicable to that approach",
      "A permanent prohibition on flying any instrument approach at a given aerodrome",
      "A rule that only applies to visual approaches, never instrument approaches",
      "A restriction that applies only to aircraft without an autopilot",
    ],
    answer: 0,
    explain:
      "An approach ban prevents a flight from continuing an instrument approach past a specified point (commonly the outer marker or an equivalent position) if the reported visibility or RVR at the time is below the minimum required for that particular approach and aircraft/crew category, rather than continuing on the chance that conditions might improve by the time minima are reached.",
    reference: "Operational Procedures - Approach Ban",
  },
  {
    id: 39,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Before commencing the take-off roll, why do crews generally cross-check the aircraft's heading indication against the charted runway heading?",
    options: [
      "To guard against inadvertently lining up on or departing from the wrong runway",
      "To verify the accuracy of the destination's forecast winds",
      "To calibrate the standby altimeter before flight",
      "It is done purely as a formality with no safety purpose",
    ],
    answer: 0,
    explain:
      "Cross-checking the aircraft's heading indication against the runway's charted magnetic heading just before take-off is a simple, effective safeguard against lining up on the wrong runway (or the correct runway pointing the wrong way) - a mistake that has contributed to serious accidents in aviation history and is one of the reasons this check is a standard item in take-off procedures.",
    reference: "Operational Procedures - Runway Alignment Verification",
  },
  {
    id: 40,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "After a high-energy rejected take-off (a reject at or near maximum speed), why might the aircraft's brakes require attention before the next flight?",
    options: [
      "The braking effort generates significant heat, and the brakes may need to be inspected and allowed to cool before further operation",
      "The brakes are automatically replaced after every rejected take-off regardless of speed",
      "There is generally no need for any inspection, since brakes are not affected by a rejected take-off",
      "Only the tyres need attention; the brakes themselves are unaffected by a high-speed stop",
    ],
    answer: 0,
    explain:
      "Stopping a heavy aircraft from a high speed converts a large amount of kinetic energy into heat at the brakes, which may need a cooling period (and possibly an inspection, depending on how much energy was absorbed) before the aircraft is dispatched again - continuing to operate on overheated brakes without adequate cooling risks further damage or reduced brake performance.",
    reference: "Operational Procedures - Rejected Take-off Brake Considerations",
  },
  {
    id: 41,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Why is 'subtle' flight crew incapacitation generally considered harder to recognise than 'obvious' incapacitation?",
    options: [
      "The affected crew member may still appear conscious and capable while their actual performance and judgement are significantly impaired",
      "Subtle incapacitation always occurs without any change in the crew member's appearance or behaviour at all",
      "Obvious incapacitation is actually harder to detect than subtle incapacitation",
      "Subtle incapacitation only affects cabin crew, never flight crew",
    ],
    answer: 0,
    explain:
      "Obvious incapacitation (sudden collapse or unresponsiveness) is immediately apparent, but subtle incapacitation can leave a crew member appearing awake and outwardly functioning while their actual performance, judgement, or awareness is significantly degraded - making it considerably harder for the other crew member to notice without deliberately cross-checking responses and behaviour.",
    reference: "Operational Procedures - Crew Incapacitation",
  },
  {
    id: 42,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Is a passenger safety briefing generally required before every take-off, even for passengers who fly frequently?",
    options: [
      "Yes - a safety briefing is required before every take-off regardless of how experienced or frequent the passengers are",
      "No - frequent flyers are generally exempt from receiving any safety briefing",
      "Only required on an aircraft's very first flight of the day",
      "Only required if requested by a passenger",
    ],
    answer: 0,
    explain:
      "A safety briefing covering essentials such as seatbelts, exits, and emergency equipment is required before every take-off, regardless of how often a given passenger has flown - since a briefing addresses that specific aircraft/flight and cannot be assumed to be already known correctly by any individual passenger.",
    reference: "Operational Procedures - Passenger Safety Briefings",
  },
  {
    id: 43,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Before opening or selecting an emergency exit for an evacuation, cabin crew are generally trained to first:",
    options: [
      "Assess conditions outside that exit (such as fire or an obstruction) before opening it or directing passengers to use it",
      "Open every exit simultaneously without any assessment, regardless of outside conditions",
      "Wait for a specific instruction from the flight deck before assessing any exit",
      "Assess only the exit closest to their own seat, ignoring any other exits",
    ],
    answer: 0,
    explain:
      "Cabin crew are trained to check conditions outside an exit - for signs of fire, water, or a serious obstruction - before opening it or directing passengers toward it, since a hazard outside an unusable exit can make evacuating through it more dangerous than directing passengers to a different, safer exit.",
    reference: "Operational Procedures - Emergency Evacuation",
  },
  {
    id: 44,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "On encountering unexpected turbulence in cruise, what is generally the crew's first priority?",
    options: [
      "Securing the cabin (seatbelt sign on, halting cabin service) and, if appropriate, requesting a different altitude or route from ATC",
      "Immediately disconnecting the autopilot and hand-flying regardless of conditions",
      "Increasing speed to minimise time spent in the turbulent air",
      "Descending immediately without first coordinating with ATC",
    ],
    answer: 0,
    explain:
      "The immediate priority on encountering turbulence is generally to secure the cabin - seatbelt sign on and cabin service paused - to reduce injury risk, while considering (and coordinating through ATC as needed) whether a different altitude or route would provide smoother conditions, rather than an uncoordinated, unilateral change.",
    reference: "Operational Procedures - Turbulence Encounter Procedures",
  },
  {
    id: 45,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general purpose of a Configuration Deviation List (CDL)?",
    options: [
      "To list specific minor external (non-structural) parts that may be missing for dispatch, under specified conditions and limitations",
      "To list every possible internal system that could ever fail",
      "To replace the Minimum Equipment List entirely",
      "To specify which flight crew qualifications are required for a given aircraft type",
    ],
    answer: 0,
    explain:
      "A Configuration Deviation List identifies specific external, non-structural aircraft parts (such as certain small fairings or access panels) that may be missing for a flight under defined conditions and limitations (often with a performance penalty applied), separate from - but used alongside - the Minimum Equipment List, which addresses inoperative systems and equipment rather than physically missing parts.",
    reference: "Operational Procedures - Configuration Deviation List",
  },
  {
    id: 46,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is generally meant by a 'critical phase of flight'?",
    options: [
      "Taxi, take-off, landing, and other phases (such as low-altitude flight) during which crew workload and consequence of error are especially high",
      "Only the portion of the flight above FL350",
      "Any phase during which the autopilot is engaged",
      "Only the time spent parked at the gate",
    ],
    answer: 0,
    explain:
      "Critical phases of flight generally include taxi, take-off, initial climb, final approach, and landing, along with other low-altitude operations - periods with high workload and little margin for distraction or error - which is part of why procedures such as the sterile flight deck rule are specifically tied to these phases.",
    reference: "Operational Procedures - Critical Phases of Flight",
  },
  {
    id: 47,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general purpose of a Quick Reference Handbook (QRH)?",
    options: [
      "To provide a quick-access reference to abnormal and emergency checklists during flight",
      "To provide the full aircraft flight manual in an abbreviated summary form only",
      "To list scheduled maintenance intervals for the aircraft",
      "To provide passenger service standards for cabin crew",
    ],
    answer: 0,
    explain:
      "A QRH is organised for fast, reliable access to the checklists a crew needs when responding to an abnormal or emergency condition in flight, structured so that critical items can be found and followed quickly under time pressure, rather than searching through a much larger reference document.",
    reference: "Operational Procedures - Quick Reference Handbook",
  },
  {
    id: 48,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general distinction between an 'abnormal' procedure and an 'emergency' procedure?",
    options: [
      "Abnormal procedures address non-normal conditions where more time is typically available; emergency procedures address situations requiring immediate action to protect safety",
      "The two terms are simply interchangeable, with no meaningful distinction",
      "Abnormal procedures apply only on the ground; emergency procedures apply only in flight",
      "Emergency procedures are always shorter than abnormal procedures",
    ],
    answer: 0,
    explain:
      "Abnormal procedures generally address conditions that are not normal but still allow some time to work through a checklist methodically, while emergency procedures address situations serious enough to demand immediate action, sometimes even before referring to a checklist at all - a distinction reflected in how checklists are structured and prioritised.",
    reference: "Operational Procedures - Checklist Philosophy",
  },
  {
    id: 49,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general purpose of standard altitude callouts during an approach (such as calling out height above the runway at defined points)?",
    options: [
      "To support situational awareness and monitoring, helping the crew make a timely stabilized-approach or go-around decision",
      "To satisfy a purely administrative requirement with no operational benefit",
      "To replace the need for the crew to monitor flight instruments directly",
      "To signal to the cabin crew that landing is imminent",
    ],
    answer: 0,
    explain:
      "Standard altitude callouts give both pilots a shared, verbalised sense of progress down the approach at defined points, supporting cross-monitoring and helping ensure that if the approach isn't stabilized or safe to continue, that's recognised and acted on in good time rather than too late.",
    reference: "Operational Procedures - Approach Callouts",
  },
  {
    id: 50,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Ground de-icing/anti-icing is often described as a 'two-step' process. What does this generally involve?",
    options: [
      "A first step to remove existing snow, ice, or frost contamination, followed by a second step applying a fluid to protect the surfaces for a period afterward",
      "Applying the exact same fluid twice in a row for redundancy",
      "One step performed only on the wings, and a second step performed only on the tail",
      "Only removing contamination, with no protective fluid application at all",
    ],
    answer: 0,
    explain:
      "Where conditions require it, a two-step process first removes any existing contamination from critical surfaces (de-icing) and then applies a protective fluid (anti-icing) intended to prevent new contamination from adhering for an estimated period afterward - the holdover time - before departure.",
    reference: "Operational Procedures - De-icing & Anti-icing Procedures",
  },
  {
    id: 51,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "How does a 'runway excursion' generally differ from a 'runway incursion'?",
    options: [
      "A runway excursion is an aircraft veering off the side or overrunning the end of the runway; a runway incursion involves an unauthorised presence on the runway by another aircraft, vehicle, or person",
      "The two terms describe exactly the same type of event",
      "A runway excursion can only happen during take-off, never during landing",
      "A runway incursion can only be caused by the aircraft itself, never by another party",
    ],
    answer: 0,
    explain:
      "A runway excursion involves an aircraft itself departing the runway surface - veering off to the side or overrunning the end - while a runway incursion involves an unauthorised presence of another aircraft, vehicle, or person on the protected runway area; the two categories describe different kinds of runway safety event.",
    reference: "Operational Procedures - Runway Safety",
  },
  {
    id: 52,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general purpose of a Safety Management System (SMS)?",
    options: [
      "To provide a systematic organisational approach to managing safety risk, including policy, risk management, assurance, and safety promotion",
      "To replace the need for individual pilot judgement entirely",
      "To function solely as a post-accident investigation tool",
      "To manage only financial and scheduling risk, not safety risk",
    ],
    answer: 0,
    explain:
      "An SMS provides a structured, organisation-wide framework for managing safety - typically covering safety policy and objectives, proactive risk identification and management, safety assurance (monitoring that controls are working), and safety promotion (training and communication) - rather than relying solely on individual judgement or reacting only after something goes wrong.",
    reference: "Operational Procedures - Safety Management Systems",
  },
  {
    id: 53,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general purpose of a pre-flight 'walk-around' inspection?",
    options: [
      "To visually check the aircraft's external condition before flight",
      "To verify the accuracy of the destination weather forecast",
      "To confirm passenger headcount against the manifest",
      "To test the aircraft's radio equipment only",
    ],
    answer: 0,
    explain:
      "A walk-around inspection is a visual external check of the aircraft - looking for obvious damage, leaks, correct control surface position, tyre condition, and similar items - conducted before flight as a straightforward, practical safeguard that complements (rather than replaces) other pre-flight checks and documentation review.",
    reference: "Operational Procedures - Pre-Flight Inspection",
  },
  {
    id: 54,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general purpose of a NOTOC (Notification to Captain)?",
    options: [
      "To inform the commander of any dangerous goods being carried aboard that specific flight",
      "To notify the captain of the passenger count only",
      "To provide the captain with the destination's weather forecast",
      "To confirm the aircraft's maintenance release status",
    ],
    answer: 0,
    explain:
      "A NOTOC specifically informs the aircraft commander of any dangerous goods loaded aboard a particular flight - their nature, quantity, and location - so the crew has the information needed to respond appropriately should an issue involving those goods arise during the flight.",
    reference: "Operational Procedures - Dangerous Goods Notification",
  },
  {
    id: 55,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "How does a 'transit' or 'turnaround' check generally differ from a full pre-flight inspection?",
    options: [
      "It is a less extensive inspection performed during a short intermediate stop, rather than the more thorough check performed before the aircraft's first flight of the day",
      "It is always more thorough than a full pre-flight inspection",
      "It applies only to cargo aircraft, never to passenger aircraft",
      "It replaces the need for any pre-flight inspection at all",
    ],
    answer: 0,
    explain:
      "A transit or turnaround check is a scaled-down inspection appropriate to a short intermediate stop - covering the items most likely to need attention between sectors - rather than repeating the fuller, more time-consuming inspection typically carried out before the aircraft's first flight of the day.",
    reference: "Operational Procedures - Transit & Turnaround Checks",
  },
  {
    id: 56,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general purpose of a 'cabin secure' report from cabin crew to the flight deck before take-off or landing?",
    options: [
      "To confirm that the cabin (doors, galleys, aisles, and similar items) is properly configured and ready for that phase of flight",
      "To confirm the exact number of meals served during the flight",
      "To report the current outside air temperature",
      "To request permission to begin boarding",
    ],
    answer: 0,
    explain:
      "A cabin secure report confirms to the flight deck that the cabin has been checked and is properly configured for take-off or landing - items such as doors, galley equipment, and aisles being clear and stowed - giving the flight crew confirmation that this part of the pre-take-off or pre-landing preparation is complete.",
    reference: "Operational Procedures - Cabin Crew Coordination",
  },
  {
    id: 57,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general purpose of a pre-departure briefing between flight crew members before starting the take-off roll?",
    options: [
      "To establish shared expectations about the planned departure, relevant procedures, and contingencies before the take-off begins",
      "To discuss matters entirely unrelated to the flight, as a way to relax before departure",
      "To replace the need for any written checklist",
      "To formally hand over control of the aircraft between pilots",
    ],
    answer: 0,
    explain:
      "A pre-departure briefing lets the crew agree in advance on the planned departure procedure, relevant speeds and altitudes, and how they intend to handle foreseeable contingencies (such as an engine failure after V1) - so that if something does happen, the response has effectively already been discussed rather than being worked out for the first time under pressure.",
    reference: "Operational Procedures - Pre-Departure Briefing",
  },
  {
    id: 58,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Why is a 'sterile' ramp/apron environment generally maintained around an aircraft during pushback and engine start?",
    options: [
      "To reduce distraction and hazard exposure for ground personnel during a critical, higher-risk ground operation",
      "To prevent passengers from viewing the aircraft from the terminal",
      "To reduce fuel consumption during the pushback itself",
      "It is purely a customs and security requirement with no safety purpose",
    ],
    answer: 0,
    explain:
      "Pushback and engine start involve moving machinery, rotating propellers or engine intakes, and coordinated communication between ground crew and the flight deck - keeping unnecessary activity and distraction to a minimum around the aircraft during this phase reduces the risk of injury to ground personnel or damage to equipment.",
    reference: "Operational Procedures - Ground Operations Safety",
  },
  {
    id: 59,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general difference between a 'MAYDAY' call and a 'PAN PAN' call?",
    options: [
      "MAYDAY signals a distress condition (grave and imminent danger requiring immediate assistance); PAN PAN signals an urgency condition that concerns safety but is not immediately life-threatening",
      "The two calls mean exactly the same thing and are used interchangeably",
      "PAN PAN is more serious than MAYDAY",
      "MAYDAY is only used for medical emergencies, and PAN PAN for all other issues",
    ],
    answer: 0,
    explain:
      "MAYDAY (repeated three times) declares a distress condition - grave and imminent danger requiring immediate assistance - while PAN PAN (also repeated three times) declares an urgency condition, one that concerns the safety of the aircraft or people aboard but does not represent an immediate, grave threat - a graduated way of communicating the seriousness of a situation to ATC and other traffic.",
    reference: "Operational Procedures - Distress & Urgency Radio Calls",
  },
  {
    id: 60,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general distinction between a 'ditching' and a 'forced landing'?",
    options: [
      "Ditching refers to a controlled emergency landing on water; a forced landing refers to an unplanned landing on land",
      "The two terms describe exactly the same event, just using different words",
      "A ditching can only occur intentionally during flight testing, never as an emergency",
      "A forced landing can only happen over water, never over land",
    ],
    answer: 0,
    explain:
      "Ditching specifically describes a controlled emergency landing carried out on water, while a forced landing describes an unplanned landing carried out on land - the two terms distinguish the surface being landed on, since the techniques, equipment (life vests, rafts), and preparation involved differ significantly between them.",
    reference: "Operational Procedures - Ditching & Forced Landing",
  },
  {
    id: 61,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Before a planned ditching, what is a passenger brace position generally intended to help reduce?",
    options: [
      "The risk of injury from impact forces and flailing during the landing",
      "The risk of hypoxia during the descent",
      "The risk of hearing damage from cabin noise",
      "The aircraft's structural loading during the flare",
    ],
    answer: 0,
    explain:
      "A brace position is designed to reduce injury from impact forces and uncontrolled flailing during a hard or emergency landing (including a ditching), by positioning the body to better withstand the deceleration forces involved and reduce the chance of striking nearby surfaces.",
    reference: "Operational Procedures - Ditching & Forced Landing",
  },
  {
    id: 62,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What type of extinguishing agent is commonly carried in aircraft cabin portable fire extinguishers, given its effectiveness across multiple fire types in an enclosed space?",
    options: [
      "A halon-type (halogenated hydrocarbon) agent",
      "Plain water only, with no other agent used",
      "Sand only, carried in a dedicated container",
      "Compressed carbon dioxide only, in every installation",
    ],
    answer: 0,
    explain:
      "Halon-type extinguishing agents have been widely used in aircraft cabins because they are effective against multiple classes of fire, leave minimal residue, and don't pose the same electrical conductivity or cleanup concerns as water, which is important in a confined cabin environment - though the type and design of extinguishers carried can vary by aircraft and operator.",
    reference: "Operational Procedures - Cabin Fire Extinguishing Agents",
  },
  {
    id: 63,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "An Emergency Locator Transmitter (ELT) is generally designed to:",
    options: [
      "Activate automatically upon a sufficiently severe impact, transmitting a distress signal to aid search and rescue",
      "Only function if manually switched on by a crew member before impact",
      "Transmit continuously throughout every normal flight, regardless of any emergency",
      "Serve only as a two-way voice radio for routine ATC communication",
    ],
    answer: 0,
    explain:
      "An ELT is generally designed to trigger automatically from the deceleration forces of a sufficiently severe impact, transmitting a distress signal to help search and rescue services locate the aircraft, even if the crew is unable to manually activate it themselves - though most ELTs can also be manually triggered where needed.",
    reference: "Operational Procedures - Emergency Locator Transmitters",
  },
  {
    id: 64,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "If an in-flight fire is suspected in an area that is not directly accessible to the crew (such as a cargo compartment), what is generally the recommended course of action?",
    options: [
      "Follow the applicable fire checklist and divert to land at the nearest suitable aerodrome as soon as practical",
      "Continue to the original destination regardless of distance, since diverting wastes fuel",
      "Ignore the indication unless smoke is also visible in the cabin",
      "Increase cruise altitude to reduce the fire's oxygen supply",
    ],
    answer: 0,
    explain:
      "A suspected fire in an inaccessible area (such as cargo) is treated as a serious, time-critical threat precisely because it can't be directly fought by the crew - the standard response is to follow the relevant checklist and land as soon as possible at the nearest suitable aerodrome, rather than continuing to the original destination or waiting for further confirmation.",
    reference: "Operational Procedures - In-Flight Fire Response",
  },
  {
    id: 65,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Modern transport aircraft normally extend and retract the landing gear using which power source, with what type of backup typically provided if that system fails?",
    options: [
      "Hydraulic power normally operates the gear, with an alternate (often gravity/free-fall assisted) extension system provided as a backup for gear-down emergencies",
      "Landing gear is normally operated by electric motors alone, with no backup extension system provided",
      "Landing gear is normally extended by ram air pressure alone, with hydraulics only ever used as a backup",
      "Landing gear position is fixed and cannot be retracted on any transport aircraft",
    ],
    answer: 0,
    explain:
      "Transport aircraft landing gear is normally raised and lowered by hydraulic power; if the normal system fails, an alternate extension method - often using gravity assisted by released uplocks, sometimes helped by an accumulator or manual pump - is provided so the gear can still be lowered (though not necessarily retracted) for a safe landing.",
    reference: "Operational Procedures - Landing Gear Systems",
  },
  {
    id: 66,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Why are aircraft tires generally inflated with nitrogen rather than ordinary compressed air?",
    options: [
      "Nitrogen is inert and dry, reducing the risk of an internal tire fire from heat/oxidation and avoiding the pressure fluctuations moisture in ordinary air could cause with temperature changes",
      "Nitrogen is heavier than air and therefore keeps the tire firmer",
      "Nitrogen improves the tire's grip on a wet runway",
      "Nitrogen is used purely because it is cheaper than compressed air",
    ],
    answer: 0,
    explain:
      "Nitrogen is inert (won't support combustion) and essentially free of moisture, which reduces the risk of an internal tire fire or failure from heat generated during braking, and avoids the pressure fluctuations that trapped moisture in ordinary compressed air could otherwise cause as temperature changes.",
    reference: "Operational Procedures - Tires",
  },
  {
    id: 67,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "What is the general purpose of an aircraft's antiskid (anti-lock) braking system?",
    options: [
      "It automatically reduces or releases brake pressure on a wheel that is about to skid or lock, preserving braking friction and reducing the risk of a flat-spotted or blown tire",
      "It applies maximum brake pressure to all wheels simultaneously regardless of runway condition",
      "It is used only to keep the wheels spinning in flight for balance",
      "It disables the brakes entirely below a set groundspeed",
    ],
    answer: 0,
    explain:
      "A skidding (locked) tire loses much of its braking friction and can be badly worn through a single spot; an antiskid system senses an impending skid on each wheel and automatically modulates brake pressure to keep it near the point of maximum braking friction, improving stopping performance and protecting the tires.",
    reference: "Operational Procedures - Antiskid Systems",
  },
  {
    id: 68,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Compared with older steel brakes, what advantage do carbon brakes generally offer on transport aircraft?",
    options: [
      "Significantly lower weight along with a greater capacity to absorb and dissipate heat energy at high temperatures",
      "They never require replacement over the life of the aircraft",
      "They function identically to steel brakes but are only ever fitted to the nose gear",
      "They provide no operational advantage and are used only for cost reasons",
    ],
    answer: 0,
    explain:
      "Carbon brakes are considerably lighter than equivalent steel brakes and can absorb and dissipate more heat energy at higher temperatures, which is valuable for coping with the very high kinetic energy that must be converted to heat during a high-speed rejected take-off or a maximum-effort landing stop.",
    reference: "Operational Procedures - Brake Systems",
  },
  {
    id: 69,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Why do transport aircraft generally have multiple, largely independent hydraulic systems rather than a single system?",
    options: [
      "So that flight controls, landing gear, and brakes generally remain operable (fully or through a backup means) even if one hydraulic system is lost",
      "Because a single hydraulic system could not generate sufficient pressure for any one function",
      "Purely to reduce the total weight of hydraulic fluid carried",
      "Because regulations require the systems to be interchangeable with the fuel system",
    ],
    answer: 0,
    explain:
      "Having two or more largely independent hydraulic systems - often further backed up by an electrically or air-driven auxiliary pump - means the loss of any single system still leaves enough hydraulic power available, directly or through a backup means, to retain adequate control of the aircraft, extend the landing gear, and brake safely.",
    reference: "Operational Procedures - Hydraulic System Redundancy",
  },
  {
    id: 70,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Hot, high-pressure air bled from the engine compressors (the pneumatic system) is generally used for which of the following?",
    options: [
      "Cabin air conditioning and pressurization, wing/engine anti-icing, and (on many aircraft) engine starting",
      "Powering the primary flight control surfaces on all aircraft types",
      "Cooling the engine's turbine section only, with no other use",
      "Providing thrust for reverse thrust operation",
    ],
    answer: 0,
    explain:
      "Bleed air tapped from the engine compressor stages is a versatile shared resource, commonly used to supply the air conditioning/pressurization packs, to provide hot air for wing and engine anti-icing, and (via an air-driven starter) to help start the engines themselves.",
    reference: "Operational Procedures - Pneumatic (Bleed Air) System",
  },
  {
    id: 71,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "In general terms, what is the typical sequence of actions in an engine fire drill once a fire is confirmed?",
    options: [
      "Close the affected thrust lever to idle/cut-off, shut off fuel (and other relevant supplies) to that engine, and discharge a fire extinguishing agent into it, confirming the fire indication clears",
      "Immediately shut down both engines regardless of which one is actually affected",
      "Increase thrust on the affected engine in an attempt to blow out the fire",
      "Wait for the fire to self-extinguish before taking any action",
    ],
    answer: 0,
    explain:
      "A generic engine fire drill isolates the affected engine - closing its thrust lever and shutting off fuel and other feeds to it - and then discharges an onboard fire extinguishing agent into it, with the crew confirming from cockpit indications that the fire warning has cleared before deciding on further action such as a diversion.",
    reference: "Operational Procedures - Engine Fire Drill",
  },
  {
    id: 72,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "During the landing flare, what tendency does 'ground effect' typically produce that pilots must account for?",
    options: [
      "A tendency for the aircraft to 'float' further than expected before touching down, since reduced induced drag near the surface makes it reluctant to decelerate and settle",
      "A tendency for the aircraft to sink abruptly the moment it enters ground effect",
      "A tendency for the nose to pitch up uncontrollably regardless of any pilot input",
      "No noticeable effect on the flare at all",
    ],
    answer: 0,
    explain:
      "As the aircraft descends into ground effect close to the runway, reduced induced drag means it decelerates and sheds excess speed/energy more slowly than expected, which can produce a longer-than-expected float during the flare if the approach was flown a little fast - something pilots are trained to anticipate and manage.",
    reference: "Operational Procedures - Ground Effect in the Flare",
  },
  {
    id: 73,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "When planning a crosswind approach and landing in gusty wind conditions, what is a commonly applied technique regarding approach speed?",
    options: [
      "Add a portion of the reported gust increment (commonly around half the gust value, up to a defined maximum) to the normal reference approach speed",
      "Reduce the reference approach speed below its normal value to compensate for the gusts",
      "Fly the approach at the stalling speed to minimise the effect of gusts",
      "Gust conditions require no adjustment to the planned approach speed",
    ],
    answer: 0,
    explain:
      "A widely used technique is to add a portion of the gust increment - commonly around half the difference between the steady wind and the gust peak, subject to a defined maximum addition - to the normal reference approach speed, giving an extra margin above the stall in case of a sudden windspeed loss.",
    reference: "Operational Procedures - Gust Additive Technique",
  },
  {
    id: 74,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "For wake turbulence purposes, what is the general difference between 'radar' (distance-based) and 'non-radar' (time-based) separation minima?",
    options: [
      "Radar separation is expressed as a minimum distance between aircraft, while non-radar separation is instead expressed as a minimum elapsed time, used where radar-derived distance information isn't available",
      "Radar separation only applies at night, while non-radar separation applies during the day",
      "The two methods are identical and interchangeable at all times",
      "Non-radar separation applies only to aircraft below 10,000 feet",
    ],
    answer: 0,
    explain:
      "Where radar coverage allows an aircraft's distance from preceding traffic to be measured directly, wake turbulence separation is applied as a minimum distance; where that isn't available, an equivalent minimum time interval is used instead, since time is what controllers can still assure without radar-derived distance information.",
    reference: "Operational Procedures - Wake Turbulence Separation Methods",
  },
  {
    id: 75,
    subject: "ATPL General Knowledge",
    system: "Operational Procedures",
    q: "Compared with using full reverse thrust, what is a common operational reason for selecting only idle (or reduced) reverse thrust during a normal landing roll on a dry runway?",
    options: [
      "To reduce noise, engine wear, and the risk of ingesting debris blown up from the runway, since wheel braking alone is normally sufficient stopping power on a dry runway",
      "Because idle reverse thrust actually produces more stopping force than full reverse thrust",
      "Because full reverse thrust is prohibited by regulation on every landing",
      "Because idle reverse is required to prevent the engines from flaming out",
    ],
    answer: 0,
    explain:
      "On a dry runway with a normal landing distance margin, wheel brakes alone are usually sufficient to stop the aircraft, so many operators favour idle or reduced reverse thrust to reduce noise, engine wear, and the chance of the engines ingesting sand, stones, or other debris thrown up from the runway surface - reserving full reverse thrust for shorter or contaminated runways.",
    reference: "Operational Procedures - Reverse Thrust Technique",
  },
];
