...
import { AIRLINES, SHARED_MATERIAL, type Airline } from "../data/sharedMaterial";
      ...
      {entries.length === 0 ? (
        <div className="card">
          <div className="progress__empty">
            No shared material for {activeAirline} yet — check back soon.
          </div>
        </div>
      ) : (
        <div className="interview-list">
          {entries.map((entry) => (
            <div key={entry.id} className="card interview-item">
              <div className="shared-entry__title">{entry.title}</div>
