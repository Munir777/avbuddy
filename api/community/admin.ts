...
// the flagged-message queue, and moderation actions. See index.ts for
// the full consolidation story and the revert procedure.
//
// GET ?resource=settings|reports.
// POST body.op="settings" (same fields the GET-side settings call
//   returns: enabled / room / status) or body.op="moderate" (that
//   action -- delete_message/clear_flag/mute_user/unmute_user -- is its
//   own `action` field inside the body, unrelated to this `op` field,
//   see api/community/_adminModerate.ts).
//
